...
export class Tracker {
  ...
  /** Records completed work, moves its maintenance baseline, and optionally creates its linked expense. */
  public async markMaintenanceDone(itemId: string, input?: { date?: string; mileage?: number; cost?: number; category?: string; description?: string }): Promise<void> { const row = await this.rowById("maintenance", itemId); const carRow = (await this.sheets.list("carInfo"))[0]; const date = input?.date || today(); const mileage = input?.mileage ?? carRow?.data.currentMileage ?? 0; requireDate(date, "Service date"); requireNonNegative(mileage, "Service mileage"); const description = input?.description?.trim() || `Maintenance: ${row.data.name}`; await this.sheets.update("maintenance", row.rowNumber, { ...row.data, lastServiceDate: date, lastServiceMileage: mileage }); const car: CarInfo = { id: carRow?.data.id || id(), currentMileage: mileage, updatedAt: new Date().toISOString() }; if (carRow) await this.sheets.update("carInfo", carRow.rowNumber, car); else await this.sheets.append("carInfo", car); await this.sheets.append("serviceHistory", { id: id(), date, mileage, description, createdAt: new Date().toISOString() }); if (input?.cost && input.cost > 0) await this.addTransaction({ date, type: "expense", category: input.category || "Car Maintenance", amount: input.cost, description, paymentType: "", remarks: "" }); }
